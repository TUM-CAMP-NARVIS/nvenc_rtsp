...
namespace RK {
    ...
    bool RtspPlayer::getIPFromUrl(std::string url, char *ip, unsigned short *port) {
        ...
        int dstport = 0;
        int field = sscanf_s(url.c_str(), "rtsp://%d.%d.%d.%d:%d", &dstip[0], &dstip[1], &dstip[2], &dstip[3], &dstport);
        if (field < 4) {
    ...
    bool RtspPlayer::HandleVideoSetup(const char *buf, ssize_t bufsize) {
        ...
        const char natpacket[] = {0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
